...
// Output first 50 records
OUTPUT(CHOOSEN(gsecData, 50), NAMED('file_sample'));

// Output the number of rows in the dataset
OUTPUT(COUNT(gsecData), NAMED('row_count'));

/******************************************************************************

Conceptually equal to:

    SELECT * FROM flight_schedule_2019 LIMIT 50;
    SELECT COUNT(*) FROM flight_schedule_2019;
